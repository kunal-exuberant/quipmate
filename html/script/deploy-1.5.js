...
var deploy = (function(){	
		...
		function friend_deploy(data)
		{
			$.each(data.action,function(index,value){
				...
				if(value.status == 0)
				{
					$('#'+value.profileid).append('<div style="float:right;"><input id="'+value.profileid +'" class="add_friend" style="width:7em;height:2.2em;background:#4C66A4;color:#fff;font-size:1.1em;font-weight:bold;cursor:pointer;border: 0.1em solid #FFFFFF;" type="submit" value="+Friend" "/><div>');
				}
			});
		}
		...
		{  
			$.each(data.action,function(index,value){
				...
				if(value.status == '0')
				{
					$('#'+value.profileid).children().eq(1).append('<input class="profile_actions_button" onclick="action.add_friend(this,'+value.profileid+')" style="width:7em;" type="submit" value="+Friend" id="'+value.profileid+'"/>');
				}	
			});
		}
