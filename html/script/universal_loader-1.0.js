	...
	if(profileid == myprofileid)
	{
			...
			$('#center').append('<div id="inbox_container"></div>');
				...
				if(data.ack)
				{
					...
					if((value.message).length > 23) value.message = (value.message).slice(0,20)+'...';
					 ...
					 $('#'+showuser).append('<div style="margin-left:6em;margin-top:.5em;color:gray;"><img src="png/clock.png" width="6" /><span style="color:gray;" data="'+value.time+'">'+time_difference(value.time)+'</span></a></div>');
					if(index == 0)
					{
							$('#inbox_container').html('');
							user = showuser;
							$('#inbox_container').append('<h1 style="padding:1em;">'+data.name[user]+'</h1>');
						...
						$('#inbox_container').append('<h1 style="padding:1em;">'+data.name[user]+'</h1>');
						  ...
						  $('.sendbox').live('keypress',function(event){
							...
							{ 
								...
								me.attr('value','');
								var param = {"profileid":profileid,"userid":user,"message":message,"name":name,"photo":photo}
								$.postJSON('/chat/chat_new',param,function(data){
									me.parent().children().eq(3).append('<div class="message_each"><img class="message_each_photo" title="'+name+'" height="50" src="'+photo+'"><span class="message_each_message">'+data.message+'</span></div>');
...
function previous_talk_message(user, chat_start, opening)
{
        ...
        $.each(data.action,function(index,value){
			$('#inbox_'+user).children().eq(3).prepend('<div class="message_each"><img class="chat_each_photo" title="'+data.name[value.actionby]+'" height="50" src="'+data.pimage[value.actionby]+'"><span class="chat_each_message">'+ui.get_smiley(ui.link_highlight(value.message))+'</span><div style="float:right;margin-top:.5em;color:gray;"><img src="png/clock.png" width="6" /><span style="color:gray;">'+value.time+'</span></a></div></div>'); 
        });
		if(opening)
		{
