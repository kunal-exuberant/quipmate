				...
				if(param.action !='')
				{
					...
					if(data.action.length > 0)
					{
						...
						switch(page)
						{
							...
							case 'bio': deploy.bio_deploy(data); break;
							...
							case 'file': deploy.file_deploy(data); break;
							case 'video': deploy.video_deploy(data); break;
		...
		if(q.indexOf('@') != '-1')
		{
			...
			function global_search(action, q, request,event)
			{
				...
				{ 
					...
					request.push($.getJSON('ajax/write.php',{action:action,q:q},function(data){
		...
		{	
			...
			function real_time()
			{
				...
				$.postJSON('/chat/real_time',param,function(data){
					...
					var req_count = 0, notice_count = 0, message_count = 0;
					if(data.count !=0 )
					{
						...
						$.each(data.action,function(index,value)
						{
								...
								{			
									...
									else
									{
										...
										$('#chat_'+value.actionid).remove();
										$('#chatbox_'+you).children().eq(3).append('<div class="chat_each" id="chat_'+value.actionid+'" onmouseover="ui.chat_time_show(this)" onmouseleave="ui.chat_time_hide(this)"><img class="chat_each_photo" title="'+data.name[value.sentby]+'" height="25" width="25" src="'+data.photo[value.sentby]+'"><div class="chat_each_message"><pre>'+ui.get_smiley(ui.link_highlight(value.message))+'</pre></div><span class="time chat_time" data="'+value.time+'">'+ui.time_difference(value.time)+'</span></div>');
