...
var ui = (function(){
			...
			function page_call(page)
			{
				...
				action.document_fetch(this,profileid);
				}
        ...
        $('#html_btn').click();
			...
			{
				$('#friend_invite_heading').append('<div class="panel_heading" >Invite A Friend To Join Quipmate</div>');
		...
		{
		   var icon_cdn = $('#icon_cdn').attr('value');
			$('#message_leave').html('<div id="message_leave_title" onclick="ui.message_leave_grow(this)"  style="height:1.5em;cursor:pointer;font-size:1.2em;padding:0.5em;background-color:#ebebeb;color:#333333;font-weight:bold;">Leave a message<img src="'+icon_cdn+'/downarrow.gif" style="float:right;" /></div>');
		}
		...
		function message_leave_grow()
		{
			...
			$('#message_leave').html('<div id="message_leave_title" onclick="ui.message_leave_shrink(this)" style="height:1.5em;cursor:pointer;font-size:1.2em;padding:0.5em;background-color:#ebebeb;color:#333333;font-weight:bold;">Leave a message<img src="'+icon_cdn+'/downarrow.gif" style="float:right;" /></div>');
			...
			function suggest_deploy(me, data)
			{
				...
				$(con_name).html('<div class="people" id="suggest_'+value.profileid+'" data="'+value.profileid+'" ><a class="ajax_nav" href="profile.php?id='+value.profileid+'"><img class="lfloat" src='+data.pimage[value.profileid]+' height="35" width="35" /></a><div class="name_35"><a style="font-weight:bold;"  class="ajax_nav" href="profile.php?id='+value.profileid+'">'+data.name[value.profileid]+'</a><div  style="cursor:pointer;padding:0.2em 0em 0em 0em;"><span class="really_add_friend" onclick="action.really_add_friend(this)" >+Friend</span></div></div></div>');
				i++;
				});
			}
			...
			function suggest_deploy_page(me, data)
			{
				...
				if(data.action.length > 0)
				{
					$.each(data.action,function(index,value){
						con_name = '#'+'suggest_container'+i;
						$(con_name).html('<div class="people" id="suggest_'+value.profileid+'" data="'+value.profileid+'" ><a class="ajax_nav" href="profile.php?id='+value.profileid+'"><img class="lfloat" src='+data.pimage[value.profileid]+' height="80" width="80" /></a><div class="name_80"><a style="font-weight:bold;" class="ajax_nav" href="profile.php?id='+value.profileid+'">'+data.name[value.profileid]+'</a><div  style="cursor:pointer;padding:0.2em 0em 0em 0em;"><span class="really_add_friend" onclick="action.really_add_friend_page(this)" >+Friend</span></div></div></div>');
						i++;
					});
				}
				else
			...
			{
				$('#uploader').html('<input type="text" id="status_box" value="" placeholder="What \'s going in your life?"/><input id="link_button" class="theme_button" type="submit" value="Share">');
				ui.enable_upload_area();
			}
			...
			{
				$('#uploader').html('<div id="main_div"><input type="hidden" value="6"/><div></div><div style="margin:0.5em;"><Strong>Select a file on your computer</strong></div><div style="margin-top:15px 0 20 0px;"><div id="photo_preview"></div><form id="pform" method="post" enctype="multipart/form-data" action="/ajax/write.php"><textarea type="text" placeholder="Say something about this file" maxlength="200" id="photo_description" name="photo_description"/></textarea><input  size="30" type="file" name="photo_box" id="photo_box" class="file_inline" /><input type="submit" name="upload" id="photo_upload_button" class="theme_button" value="Upload"><input type="hidden" id="photo_hidden_profileid" name="photo_hidden_profileid" value="'+profileid+'"/><input type="hidden" name="action" value="'+action+'"/></form></div></div>');
				ui.enable_upload_area();
			}
			function OnProgress(event, position, total, percentComplete)
			{
				...
				$('#progressbar').css({"display":"block","width":percentComplete+'%',"background":"#cccccc"});
	...
	/*		function menu(me,event,admin)
		...
		function bio_item_single_edit(container,code,value,privacy)
		{
			...
			function createChatBoxUI(user, name, is_online)
			{
				...
				if(is_online)
				$('#chatbox_'+user).children().eq(2).prepend('<img class="chatbox_online_icon" src="'+icon_cdn+'/online.png" />');
				var lastScrollTopchat = $('#chatbox_'+user).children().eq(3).get(0).scrollTop;
				$('#chatbox_'+user).children().eq(3).scroll(function(){ui.chatbox_scroll(user)});
			}
