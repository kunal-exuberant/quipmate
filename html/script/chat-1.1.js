	...
	{ 
		...
		$.postJSON('/chat/chat_new',param,function(data){
			$.each(data.action,function(index,value){
				...
				me.parent().children().eq(3).append('<div class="chat_each" id="chat_'+value.actionid+'" onmouseover="chat_time_show(this)" onmouseleave="chat_time_hide(this)"><img class="chat_each_photo" title="'+data.name[value.sentby]+'" height="25" src="'+data.photo[value.sentby]+'"><div class="chat_each_message"><pre>'+ui.get_smiley(ui.link_highlight(value.message))+'</pre></div><span class="chat_time">Now</span></div>');
			...
			$.each(data.action,function(index,value)
			{
					...
					{			
						...
						else
						{
							...
							$('#chat_'+value.actionid).remove();
							$('#chatbox_'+you).children().eq(3).append('<div class="chat_each" id="chat_'+value.actionid+'" onmouseover="chat_time_show(this)" onmouseleave="chat_time_hide(this)"><img class="chat_each_photo" title="'+data.name[value.sentby]+'" height="25" src="'+data.photo[value.sentby]+'"><div class="chat_each_message"><pre>'+ui.get_smiley(ui.link_highlight(value.message))+'</pre></div><span class="chat_time">Now</span></div>');
...
function previous_talk(user, chat_start, opening)
{
        ...
        $.each(data.action,function(index,value){
			$('#chat_'+value.actionid).remove();
			$('#chatbox_'+user).children().eq(3).prepend('<div class="chat_each" id="chat_'+value.actionid+'" onmouseover="chat_time_show(this)" onmouseleave="chat_time_hide(this)"><img class="chat_each_photo" title="'+data.name[value.actionby]+'" height="25" src="'+data.pimage[value.actionby]+'"><div class="chat_each_message"><pre>'+ui.get_smiley(ui.link_highlight(value.message))+'</pre></div><span class="chat_time">'+value.time+'</span></div>'); 
        });
		if(opening)
		{
