...
class Query(Database):
	...
	def chat_insert(self, actionid, sentby, sentto, message,isfile, time):
		cursor = self.connect()
		message = unicode(message,"utf-8")
		sql_query = r"insert into inbox(ACTIONID, ACTIONBY, ACTIONON, MESSAGE,file, TIME) values('%s', '%s', '%s', '%s', '%s', '%s')" %(actionid, sentby, sentto, message,isfile, time)
		return self.query(sql_query, cursor)
	...
	def notice_unread_count(self, profileid,last_poll_time):
		cursor = self.connect()
		sql_query = "select count(*) from notice where READBIT ='0' and PROFILEID = '%s' and ACTIONBY <> '%s' and unix_timestamp(TIMESTAMP) > '%s' " %(profileid, profileid,last_poll_time)
		return self.query(sql_query, cursor)	
...
class ChatNewHandler(BaseHandler, ChatMixin):
    ...
    def post(self): 
			...
			action = []
			chat['ack']=1
			valid_formats = ['.pdf','.pptx','.ppt','.docx','.doc','.txt','.xls','.xlsx','.ods','.one','.pps','.ps','.rtf','.msg','.pptm','.png','.jpeg','.jpg']
			__UPLOADS__ = "/var/www/html/uploads/"
			self.DB_NAME = self.get_argument("database")
...
############################################################################################
			     ...
			     chat['ack']= 0
			     chat['error']="Not valid file format"
			     action.append(chat)
#Cname is a unique name generated by below function , It can be usefull if we want to hide the name of file.
...
#            		   print cname
			   ...
			   isfile = 0 
			if chat['ack'] == 0:
			   self.finish(dict(action=action))	
			else:   
				cursor = self.get_globalid()
				rows = cursor.fetchall()
				for row in rows:
					self.chat_insert(row[0], self.get_argument("profileid"), self.get_argument("userid"), MySQLdb.escape_string(message),isfile,time.time())
					chat['actionid'] = row[0]
