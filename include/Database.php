...
class Database
{
   ...
   function event_settings_save($eventid, $invite, $privacy, $desc, $date, $timing,
      $venue)
   {
   ...
   function event_create($actionid, $invite, $privacy, $name, $desc, $date, $timing,
      $venue, $createdby, $time, $groupid = 0)
   {
      $actionid = strip_tags($this->con->real_escape_string($actionid));
      $invite = strip_tags($this->con->real_escape_string($invite));
      $privacy = strip_tags($this->con->real_escape_string($privacy));
...
LIMIT 0 , 30");
   ...
   function visible_check($actionid)
   {
      ...
      if($res['VISIBLE'] == 0 || $res['VISIBLE'] == -1 || $res['VISIBLE'] == -2)
      {
         ...
         if(($_SESSION['SEX'] == 1 && $res['VISIBLE'] == -3) || ($_SESSION['SEX'] == 0 &&
            $res['VISIBLE'] == -4))
         {
            return $res;
         }
         else
            if($res['VISIBLE'] == -5 && $myprofileid == $res['ACTIONBY'])
            {
               return $res;
            }
            else
   ...
   */
   ...
   function get_actionid($diaryid, $actiontype, $pageid = 0, $visible_id = 0, $readbit =
      0)
   {
      $myprofileid = $_SESSION['USERID'];
      $diaryid = strip_tags($this->con->real_escape_string($diaryid));
   ...
   function notice_insert($actionid, $diaryid, $actiontype, $pageid, $myprofileid =
      0)
   {
      if($myprofileid == 0)
      {
...
FROM action as A INNER JOIN (SELECT MAX(ACTIONID)  AS ACTIONID FROM action INNER JOIN subscribe as sub ON CASE WHEN action.PROFILEID <1000000000 THEN action.PROFILEID ELSE action.ACTIONBY END = sub.FRIENDID INNER JOIN actiontype on actiontype.actiontypeid = action.ACTIONTYPE WHERE sub.PROFILEID='$profileid' AND actiontype.poll_feed ='1' AND UNIX_TIMESTAMP(action.TIMESTAMP) > '$time'  GROUP BY PAGEID ORDER BY ACTIONID DESC) AS B ON A.ACTIONID = B.ACTIONID ");
   ...
   function bio_insert($profileid, $email, $name, $fname, $mname, $lname, $gender,
      $birthday)
   {
      $profileid = $this->con->real_escape_string($profileid);
      $email = $this->con->real_escape_string($email);
   ...
   {
      $profileid = $this->con->real_escape_string($profileid);
      $result = $this->con->query("SELECT FRIENDID FROM friend WHERE PROFILEID = '$profileid' UNION ALL SELECT PROFILEID as FRIENDID from friend_request where FRIENDID ='$profileid'");
      return $result;
   }
   ...
   function friend_invite_insert($actionid, $profileid, $myprofileid)
   {
      ...
      return $this->con->query("INSERT INTO subscribe(PROFILEID,FRIENDID) VALUES('$myprofileid','$profileid')");
   }
   ...
   function guest_status($profileid1, $profileid2)
   {
      ...
      if($frow)
      {
         if($frow['priviledge'] == 1)
         {
            ...
            if($frow['priviledge'] == 0)
            {
               return 2;
            }
            else
               if($frow['priviledge'] == 2)
               {
                  return 3;
               }
      }
   ...
   */
   ...
   {
      $k = $this->con->real_escape_string($k);
      return $this->con->query("SELECT * FROM `group` WHERE name like '%$k%' LIMIT $start,10");
   }
   ...
   function bio_skill_all_select($db_name)
   {
      return $this->con->query("SELECT i.name as skill,bh.profileid as profileid,bi.NAME as name FROM `$db_name`.`info` as i INNER JOIN `$db_name`.`bio_history` as bh on i.DIARYID = bh.diaryid INNER JOIN `$db_name`.`bio` as bi on bh.profileid = bi.PROFILEID WHERE i.type=230");
...
AND DATE_FORMAT(BIRTHDAY,'%m,%d') >= DATE_FORMAT(NOW(),'%m,%d') ORDER BY DATE_FORMAT(BIRTHDAY,'%m,%d')");
   ...
   function setting_email_complete_update($profileid, $friend_request, $profile_post,
      $post_comment, $message, $missu)
   {
      $profileid = $this->con->real_escape_string($profileid);
